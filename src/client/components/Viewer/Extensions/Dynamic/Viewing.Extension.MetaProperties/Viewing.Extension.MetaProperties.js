    ...
    const finalMetaProperties =
      metaProperties.filter((metaProperty) => {
        if (metaProperty.isOverride) {
          ...
          for (const [idx, modelProperty] of entries) {
    ...
    return (
      <div className='title'>
        ...
        <div className='meta-properties-controls'>
          ...
          <button
            onClick={() => this.showAddMetaPropertyDlg(dbId)}
            title='Add new MetaProperty on component'
          >
            <span className='fa fa-plus' />
          </button>
          <button
            onClick={() => this.deleteAllProperties(dbId)}
            title='Delete all MetaProperties on component'
          >
            <span
              className='fa fa-close' style={{
                position: 'relative',
                top: '-1px'
              }}
            />
